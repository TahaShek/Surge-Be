...
class AIService {
    constructor() {
        if (!env_1.config.GEMINI.api_key) {
            logger_1.default.warn("Gemini API key not configured. AI features will be disabled.");
            return;
        }
        this.genAI = new generative_ai_1.GoogleGenerativeAI(env_1.config.GEMINI.api_key);
...
Be encouraging and specific. Mention skill matches and opportunities.`;
    ...
    getFallbackInsights(matchData) {
        ...
        if (matchData.matchDetails.skillsMatch >= 70) {
            ...
            insights.push(`📚 Consider learning: ${matchData.matchDetails.missingSkills.slice(0, 2).join(", ")}`);
        }
...
Return ONLY valid JSON, no markdown formatting.`;
            ...
            logger_1.default.error("AI job enhancement error:", error);
            return this.getFallbackJobEnhancement(jobData);
        }
    }
    /**
...
Return ONLY valid JSON, no markdown formatting.`;
            ...
            logger_1.default.error("AI cover letter analysis error:", error);
            return this.getFallbackCoverLetterAnalysis(coverLetterData);
        }
    }
    /**
...
Make questions specific, insightful, and actionable. Return ONLY valid JSON, no markdown.`;
            ...
            logger_1.default.error("AI interview questions error:", error);
            return this.getFallbackInterviewQuestions(questionData);
        }
    }
    // Fallback methods
    getFallbackJobEnhancement(jobData) {
        return {
    ...
    getFallbackCoverLetterAnalysis(data) {
    ...
    getFallbackInterviewQuestions(data) {
        return {
            technicalQuestions: [
